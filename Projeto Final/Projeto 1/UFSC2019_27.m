...
A=m1*m2*m3*m4*m5-1; % valor m?ximo de sa?da (m?ximo DR) %
%A=1111111111
R1=mod(A,m1);
% com reducao previa modular %
R2=mod(A,m2);
% sem reducao previa modular %
R2p=mod(A,m2*27);
R2u=floor(R2p/2^n);
R2d=mod(R2p,2^n);
...
C=mod(2^(n),m2)
A
X=mod(mod(V13*R1+V23*C*R2u+V23*R2d+V33*R3+V43*R4+V53*R5,m4*m5)*m2*m3+V13x*R1+V23x*C*R2u+V23x*R2d+V33x*R3,mh1)*m1+R1
Y=(mod(V13*R1+V23*C*R2u+V23*R2d+V33*R3+V43*R4+V53*R5,m4*m5)*m2*m3+V13x*R1+V23x*C*R2u+V23x*R2d+V33x*R3)*m1+R1;

F1=V13x*R1+V23x*C*R2u+V23x*R2d+V33x*R3;
F2=mod(V13*R1+V23*C*R2u+V23*R2d+V33*R3+V43*R4+V53*R5,m4*m5)*m2*m3;
F3=mh1;
F4=(F1+F2);
F5=F4/F3 % Se ? menor que 2 ent?o posso usar a ideia
